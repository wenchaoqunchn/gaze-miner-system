...
# FONT
"""
修改：字体替换 Ubuntu -> Arial
原：FONT = {"family": "Ubuntu", "size": 12}
改：FONT = {"family": "Arial", "size": 12}
"""
...
def draw_fixations(
    ...
    if durationsize:
        """
        修改：时间单位对齐 秒 -> 毫秒
        原：siz = 1 * (fix["dur"] / 30.0)
        改：siz = 1000 * (fix["dur"] / 30.0)
        """
        # siz = 1 * (fix["dur"] / 30.0)
        siz = 1000 * (fix["dur"] / 30.0)
    else:
        """
        修改：时间单位对齐 秒 -> 毫秒
        原：siz = 1 * numpy.median(fix["dur"] / 30.0)
        改：siz = 1000 * numpy.median(fix["dur"] / 30.0)
        """
        # siz = 1 * numpy.median(fix["dur"] / 30.0)
        siz = 1000 * numpy.median(fix["dur"] / 30.0)
    if durationcolour:
        col = fix["dur"]
    else:
...
def draw_heatmap(
    ...
    """
    修改：统一为int型变量
    原：heatmapsize = dispsize[1] + 2 * strt, dispsize[0] + 2 * strt
    改：heatmapsize = (int(dispsize[1] + 2 * strt), int(dispsize[0] + 2 * strt))
    """
    ...
    for i in range(0, len(fix["dur"])):
        ...
        x = int(strt + fix["x"][i] - gwh / 2)
        y = int(strt + fix["y"][i] - gwh / 2)
        ...
        if (not 0 < x < dispsize[0]) or (not 0 < y < dispsize[1]):
            ...
            try:
                heatmap[y : y + vadj[1], x : x + hadj[1]] += (
                    gaus[vadj[0] : vadj[1], hadj[0] : hadj[1]] * fix["dur"][i]
                )
            except:
            ...
            heatmap[int(y) : int(y) + int(gwh), int(x) : int(x) + int(gwh)] += (
                gaus * fix["dur"][i]
            )
    # resize heatmap
    """
...
def draw_raw(x, y, dispsize, imagefile=None, savefilename=None):
    ...
    ax.plot(
        x, y, "o", markersize=10, color="#9FC9DF", alpha=0.8, markeredgecolor="#9FC9DF"
    )
...
def draw_scanpath(
    fixations, saccades, dispsize, imagefile=None, alpha=0.5, savefilename=None
):

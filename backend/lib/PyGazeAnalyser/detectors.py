    ...
    for i in range(1, len(x)):
        ...
        if dist <= maxdist and not fixstart:
            ...
            if time[i - 1] - Sfix[-1][0] >= mindur:
                Efix.append(
                    [Sfix[-1][0], time[i - 1], time[i - 1] - Sfix[-1][0], x[si], y[si]]
                )
...
def saccade_detection(x, y, time, missing=0.0, minlen=5, maxvel=40, maxacc=340):
    ...
    while not stop:
        ...
        sacstarts = numpy.where(
            (vel[1 + t0i :] > maxvel).astype(int) + (acc[t0i:] > maxacc).astype(int)
            >= 1
        )[0]
        if len(sacstarts) > 0:
            ...
            sacends = numpy.where(
                (vel[1 + t1i :] < maxvel).astype(int) + (acc[t1i:] < maxacc).astype(int)
                == 2
            )[0]
            if len(sacends) > 0:
